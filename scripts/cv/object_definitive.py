...
class ImageWorker:
    ...
    def __init__(self, bridge):
        ...
        self.index = 0
        self.singelModel = load_model('model/SingleJules.h5')
    ...
    def detect_shape(self, imgMes):
        ...
        if len(contours) > 0:
            for cnt in contours:
                ...
                #res = cv2.resize(imageWarped, dsize=(self.size_crop, self.size_crop), interpolation=cv2.INTER_CUBIC)
                #img_to_classify = np.reshape(res, [1, self.size_crop, self.size_crop, 3]) # last position 1 if black and white
        
                #img_to_classify = img_to_classify.astype(np.float64)
                #generator = self.nn_classifier.data_generate()
                #img_return = generator.standardize(img)
        ...
        with open('cluster_data.csv', 'a') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerows(self.list_loc_signs)
...
def main(args):
    ...
    rospy.init_node('object_definitive', anonymous=True)
    rate = rospy.Rate(20)
    ...
    #img_sub = rospy.Subscriber("/cf1/camera/image_rect_color", Image, imageCallback)
