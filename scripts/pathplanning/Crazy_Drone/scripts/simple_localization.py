...
def publish_cmd(drone_pose):
    posi = Position()

    posi.header.stamp = rospy.Time.now()
    posi.header.frame_id = drone_pose.header.frame_id

    posi.x = drone_pose.pose.position.x
    posi.y = drone_pose.pose.position.y
    posi.z = drone_pose.pose.position.z

    roll, pitch, yaw = euler_from_quaternion((drone_pose.pose.orientation.x,
                                              drone_pose.pose.orientation.y,
                                              drone_pose.pose.orientation.z,
                                              drone_pose.pose.orientation.w))

    posi.yaw = math.degrees(yaw)

    pub_loc.publish(posi)
'''    
print("POSE")
...
def main():
    ...
    pub_loc = rospy.Publisher('/cf1/cmd_position', Position, queue_size=2)
